			...
			else
			{
				...
				else
				{
					for (i = 0; i < result.length; i++)
					{
						...
						var idx = result[i]['id'];
						html += '<tr>';
                        html += `<td><a href="javascript:void(0)" onclick="javascript:openDados('${idx}')">${doc}</a></td>`;
...
function openDados(doc) {
    var token = getCookie('token');

    $.ajax({
        type: 'GET',
        url: 'Servico/Upbusca/' + doc,
	    headers: {"Authorization": "Bearer " + token},
        dataType: 'json',
        success: function(data)
        {
            $(".blockServico").html(data.body);
        }
    });
...
$(document).ready(function()
{
	...
	$("#nova_consulta").click(function(){
		try {
            ...
            location.href = "Servicos/Upbusca";
		}
	});
